...
class CountryController {
  ...
  static getCountryById = async (countryId: string) => {
    try {
      const result = await pool.query(
        "SELECT * FROM public.country WHERE id = $1",
        [countryId]
      );
  ...
  static saveCountry = async (request: any, response: any) => {
    ...
    try {
      ...
      if (result.rows.length > 0) {
        const country = result.rows[0];
        await pool.query(
          "UPDATE public.country SET name=$1, updated_at = NOW() WHERE id = $2",
          [name, id]
        );
        ...
        const newCountry = await pool.query(
          "INSERT INTO public.country (name, created_at) VALUES ($1, NOW()) RETURNING *",
          [name]
        );

        const createdCountry = newCountry.rows[0];
        const country = new Country(
          createdCountry.id.toString(),
          createdCountry.name,
          createdCountry.created_at,
          createdCountry.updated_at
        );
