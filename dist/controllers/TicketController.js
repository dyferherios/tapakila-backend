...
TicketController.getTicketById = async (ticketId) => {
    try {
        ...
        const currency = await CurrencyController.getCurrencyById(currencyId);
        const event = await EventController.getEventById(eventId);
        return new Ticket(ticket.id.toString(), event, ticketType, user, ticket.ticket_number, ticket.amount_paid, currency, ticket.payment_confirmed, ticket.created_at, ticket.updated_at);
    }
    catch (error) {
...
TicketController.saveTicket = async (request, response) => {
    try {
        ...
        if (result.rows.length > 0) {
            const ticketUpdated = await pool.query("UPDATE public.ticket SET event_id = $1, ticket_type_id = $2, user_id = $3, ticket_number = $4, amount_paid = $5, currency_id = $6, payment_confirmed = $7, updated_at=NOW() WHERE id = $8 RETURNING *", [
                eventId,
                ticketTypeId,
                userId,
                ticketNumber,
