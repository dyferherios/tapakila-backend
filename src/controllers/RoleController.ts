    ...
    try {
        ...
        if (result.rows.length > 0) {
            ...
            const newRole = await pool.query(
                'INSERT INTO role (id, title, created_at, updated_at) VALUES ($1, $2, NOW(), NOW()) RETURNING *',
                [id, title]
            );
        ...
        console.error(error);
        response.status(500).json({ error: 'An error occurred while saving/updating the role' });
    }
};
