...
class UserController {
    ...
    try {
      ...
      if (result.rows.length > 0) {
        ...
        await pool.query(
          ...
          [role_id.getId(), username, name, email, email_verified_at, password, image_url, country_id.getId(), id]
        );
        const userUpdated = await UserController.getUserById(id);
        response.status(200).json(userUpdated);
      } else {
        const newUser = await pool.query(
          'INSERT INTO public."user" (role_id, username, name, email, email_verified_at, password, image_url, country_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())',
          [
            role_id.getId(),
            username,
            name,
  ...
  static deleteUser = async (request: any, response: any) => {
