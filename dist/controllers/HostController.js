...
HostController.saveHost = async (request, response) => {
    ...
    try {
        ...
        if (result.rows.length > 0) {
            const host = result.rows[0];
            await pool.query("UPDATE host SET name = $1, description = $2, updated_at = NOW() WHERE id = $3", [name, description, id]);
            ...
            const newHost = await pool.query("INSERT INTO host (name, description, created_at, updated_at) VALUES ($1, $2 NOW(), NOW()) RETURNING *", [name, description]);
