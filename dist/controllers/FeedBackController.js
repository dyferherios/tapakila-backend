    ...
    try {
        ...
        if (id) {
            ...
            if (result.rows.length > 0) {
                ...
                const feedBack = result.rows[0];
                const updatedFeedbacks = new Feedback(feedBack.id.toString(), user, subject, message, feedBack.created_at, new Date());
            ...
            const feedBackObject = new Feedback(createdFeedback.id.toString(), userObj, createdFeedback.subject, createdFeedback.message, createdFeedback.created_at, createdFeedback.updated_at);
